  ...
  describe '#perform' do
    ...
    context 'when the deposit is too low' do
      it 'declines the application due to high Loan-to-Value' do
        ...
        expect(result[:explanation]).to include('Loan-to-Value ratio (97.5%) exceeds maximum limit of 90%')
      end
    end
    ...
    context 'when the monthly outgoings are too high' do
      it 'declines the application due to high Debt-to-Income' do
        ...
        expect(result[:explanation]).to include('Debt-to-Income ratio (60.0%) exceeds maximum limit of 45%')
      end
    end
